#### Nebujte se si sem napsat na jednu řádku k čemu všemu ten skript je ;-)

## Encoding:   windows-1250
## Vytvořil/a: 2024-06-?? AlRast
## Upravil/a:  2024-06-14 FrK

## POZNÁMKY:
## 1) Bacha! Bylo třeba udělat pár úprav v kódu na tvorbu škál, 
##    stačilo, aby tam byl jeden missing a hned celý případ vypadl z analýzy.
##    
## 2)    
## 
## 
...
df_Osob_hran <- 
  data %>% 
    ...
    # drop_na() %>% 
    mutate(Osob_hran = rowMeans(across(starts_with("Osob_hran_")), na.rm = T)) %>% 
...
df_Zmen_chov <- 
  data %>% 
    ...
    mutate(across(c(Zmen_chov_01:Zmen_chov_16), ~ recode(., 
                                         ...
                                         "Vadí" = 1))) %>% 
    # drop_na() %>% 
    mutate(Zmen_chov = rowMeans(across(starts_with("Zmen_chov_")), na.rm = T)) %>% 
...
df_Ost_hran <- 
  data %>% 
    ...
    mutate(across(c(Ost_hran_01:Ost_hran_08), ~ recode(., 
                                         ...
                                         "Rozhodně ne" = 1))) %>% 
    # drop_na() %>% 
    mutate(Ost_hran = rowMeans(across(starts_with("Ost_hran_")), na.rm = T)) %>% 
    select(-(c(Ost_hran_01:Ost_hran_08)))
...
data %>% 
  ...
  group_by(Osob_kon) %>% 
  summarise(Osob_hran = mean(Osob_hran, na.rm = T)) %>% 
  ggplot(aes(x = Osob_kon, y = Osob_hran, fill = Osob_kon, label = round(Osob_hran, 1)))+
  geom_col() +
  geom_text(position = position_stack(vjust = 0.9), color = "white") +
  theme_classic()
...
df_hranice = data %>% 
  select(Osob_hran, Ost_hran) %>% 
  mutate(rozdil = Osob_hran - Ost_hran)

df_hranice %>% 
  ggplot() +
  aes(x = Osob_hran, y = Ost_hran) +
...
df_hranice$rozdil %>% summary()
  
sjstats::t_test(df_hranice, select = "rozdil", mu = 0)  
# Hmm.... takže tam není rozdíl mezi mojí hranicí a hranicí pro druhé.
# Nemůže to být tím, že se přísně konstruovaly škály, že když byla někde
# jen jedna NA, případ celý vypadl? Zkusíme spočítat ty průměry bez 
# těch chybějících hodnot (průměr není spočtený ze všech, ale jen z platných), no?
# A vida! Když to změníme, najednou je ten průměrný rozdíl -0.1 významný!
# Statisticky, věcně nic moc, Cohenovo d je pořád malé.
